...
namespace State
{
    ...
    {
        private static readonly Dictionary<string, Data.IDataModule> _modules = new();
        private static readonly object _lock = new();

        private const string FOLDER_NAME = "State";
        ...
        {
            Plugin.Log.LogInfo("Store initialized");
        }
        ...
        /// <example>
        /// <code>
        /// Store.Register&lt;VikingPlayerData&gt;("viking");
        /// </code>
        /// </example>
        public static void Register<T>(string moduleId) where T : class, IPlayerData, new()
        {
            ...
            lock (_lock)
            {
                ...
                _modules[moduleId] = new Data.DataModule<T>(moduleId);
        ...
        public static T Get<T>(long playerId, string moduleId) where T : class, IPlayerData, new()
        {
            lock (_lock)
            {
                ...
                if (module is Data.DataModule<T> typedModule)
                {
        ...
        public static void Set<T>(long playerId, string moduleId, T data) where T : class, IPlayerData, new()
        {
            if (!Plugin.IsServer())
            {
                Plugin.Log.LogError("Store.Set() can only be called on server");
                return;
            }
            ...
            lock (_lock)
            {
                ...
                if (module is Data.DataModule<T> typedModule)
                {
                    ...
                    StateNetwork.SyncToClient(playerId, moduleId);
                }
                else
                {
